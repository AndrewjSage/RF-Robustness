...
#Function to divide real datasets into test and training data and store a singe 3-D array
#with test and training set for each rep. 
#First index is nfolds*(rep-1)+fold. (i.e. reps and folds are combined together in indexing)

DivideRealData <- function(DATA, nreps, nfolds, p=.2 ){  #p is proportion contaminated
  stdev=sd(DATA[,ncol(DATA)])
  ntest <- nrow(DATA)/nfolds
  ntrain <- nrow(DATA)-ntest
  TRAIN <- array(NA, dim=c(nreps*nfolds, ntrain, ncol(DATA)))
  TEST <- array(NA, dim=c(nreps*nfolds, ntest, ncol(DATA)))
  OutlierIndicator <- array(NA, dim=c(nreps*nfolds, ntrain))
  for(rep in 1:nreps){
    ...
    for(fold in 1:nfolds){
      ...
      TRAIN[(nfolds*(rep-1)+fold),,] <- as.matrix(DATA[CVTRAINind,,])
      TEST[(nfolds*(rep-1)+fold),,] <-  as.matrix(DATA[CVTESTind,,])  
      ds=rbinom(ntrain,1,p)  #determine which training cases are outliers
      e=c(rep(0, ntrain))   #initialize error vector
      e[ds==1]=rnorm(length(e[ds==1]),0,5*stdev) #contaminated errors for variance cont.
      TRAIN[rep,,ncol(TRAIN[rep,,])] <- TRAIN[rep,,ncol(TRAIN[rep,,])]+e
      OutlierIndicator[(nfolds*(rep-1)+fold),] <- ds
    }    
  }
